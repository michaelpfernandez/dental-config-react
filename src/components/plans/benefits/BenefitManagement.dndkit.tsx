...
const BenefitManagement: React.FC<BenefitManagementProps> = ({
  ...
  // Get limit for a specific benefit
  const getLimit = (classId: string, benefitId: string) => {
    return limitStructure?.limits?.find(
      (limit) => limit.classId === classId && limit.benefitId === benefitId
    );
  };
  ...
  const addBenefitToClass = (classId: string, benefitId: string, benefitName: string) => {
    ...
    newCostShares.push({
      ...
      values: { coinsurancePercentage: 20 },
    ...
    >
      ...
      <Box sx={{ width: '150px' }}>
        <FormControl fullWidth size="small">
          <Select
            value={costShare?.costShareType || CostShareType.Coinsurance}
            onChange={(e) =>
              handleCostShareTypeChange(classId, benefit.id, e.target.value as CostShareType)
            }
            size="small"
          >
          ...
          {isEditingLimit ? (
            <Grid container spacing={1} alignItems="center">
              <Grid item xs={5}>
                <TextField
                  ...
                  error={!!limitValidationError && limitValidationError.includes('Quantity')}
                  onChange={(e) =>
                    handleLimitChange(
                      classId,
                      benefit.id,
              ...
              <Grid item xs={5}>
                <FormControl fullWidth size="small">
                  ...
                  <Select
                    value={limit?.interval?.type || LimitIntervalType.PerYear}
                    onChange={(e) =>
                      handleLimitChange(classId, benefit.id, 'intervalType', e.target.value)
                    }
                    label="Interval"
                  >
                    {Object.values(LimitIntervalType).map((type) => (
                      <MenuItem key={type} value={type}>
                        {getEnumDisplayName(type)}
                      </MenuItem>
                    ))}
              ...
              <Grid item xs={2}>
                <IconButton
                  size="small"
                  onClick={() => toggleLimitEditing(classId, benefit.id)}
                  disabled={!!limitValidationError}
                  color="primary"
                >
            ...
            <>
              <Typography variant="body2" sx={{ flexGrow: 1 }}>
                {limit
                  ? `${limit.quantity} per ${getEnumDisplayName(limit.interval.type)}`
                  : 'No limit set'}
              </Typography>
              <IconButton size="small" onClick={() => toggleLimitEditing(classId, benefit.id)}>
                <EditIcon fontSize="small" />
              </IconButton>
            </>
          )}
        </Box>
        {limitValidationError && limitValidationError.includes('Quantity') && (
